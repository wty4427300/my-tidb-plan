...
//keuvalue是一种用于保存传递给map/reduce函数的类型
...
//jobPhase指示是否将任务调度为map任务或reduce任务。
...
type task struct {
	dataDir    string
	jobName    string
	mapFile    string   // only for map, the input file//仅用于map，输入文件
	phase      jobPhase // are we in mapPhase or reducePhase?//判断是mapphase还是reducephase
	taskNumber int      // this task's index in the current phase//这个任务在当前阶段的索引
	nMap       int      // number of map tasks//map任务数量
	nReduce    int      // number of reduce tasks//reduce任务数量
	...
	for {
		...
		case t := <-c.taskCh:
			if t.phase == mapPhase {
				...
				opFile, opBuffer := CreateFileAndBuf(mergeName(t.dataDir, t.jobName, t.taskNumber))
				for key, value := range kvs {
					WriteToBuf(opBuffer, t.reduceF(key, value))
				}
				SafeClose(opFile, opBuffer)
				//舍弃
				//var keys []string
				//for k:=range kvs{
				//	keys=append(keys,k)
				//}
				//sort.Strings(keys)
				//p:=mergeName(t.dataDir, t.jobName ,t.taskNumber)
...
func reduceName(dataDir, jobName string, mapTask int, reduceTask int) string {
	//return path.Join(dataDir, "inputMapFile"+strconv.Itoa(reduceTask))
